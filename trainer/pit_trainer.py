...
from losses import PITNetLoss

class PIT_Trainer(object):
    ...
    def run_with_feature(self):
        try:
            while True:
                self.model.train()
                for feature, mix_mag, mix_phase, s1_mag, s1_phase, s2_mag, s2_phase in self.train_dl:
                    ...
                    l = PITNetLoss(mix_mag, mix_phase, predict_mask, s1_mag, s1_phase, s2_mag, s2_phase, self.device)
                    loss = l.compute()
                    loss.backward()
                    ...
                    if train_loss > 1e8 or math.isnan(train_loss):
                        self.logger.error("Loss exploded to %.02f at step %d!" % (train_loss, self.current_step))
                        raise Exception("Loss exploded")

                    # write loss to tensorboard
                    if self.current_step % self.summary_interval == 0:
                        self.writer.log_training_v2(self.name, loss, self.current_step)
                        self.logger.info('step:{0}, loss:{1}'.format(self.current_step, train_loss))
    ...
    def run(self):
        try:
            while True:
                self.model.train()
                for mix_mag, mix_phase, s1_mag, s1_phase, s2_mag, s2_phase in self.train_dl:
                    mix_mag = mix_mag.to(self.device)
                    try:
